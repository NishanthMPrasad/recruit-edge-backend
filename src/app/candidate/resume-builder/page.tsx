...
import StrikeMark from '@tiptap/extension-strike';
...
const TiptapEditor: React.FC<EditorProps> = ({ value, onChange, placeholder }) => {
  const editor = useEditor(
    {
      ...
      editorProps: {
        attributes: {
          class:
            'prose prose-sm max-w-none text-white focus:outline-none min-h-[150px] p-3 rounded-md border border-white/20 bg-white/10',
        },
      },
      injectCSS: false,
              ...
              <label className="flex items-start space-x-3">
                ...
                <span
                  ...
                  dangerouslySetInnerHTML={{
                    __html: version === originalText ? `<strong>(Original)</strong> ${version}` : version,
                  }}
                />
              </label>
            </div>
