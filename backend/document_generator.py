...
def add_html_to_docx_paragraph(paragraph, html_content, normal_style=None):
    ...
    for element in soup.contents if soup.body else soup.contents:
        if element.name == 'ul':
            ...
            p_tag = paragraph if not paragraph.text else doc.add_paragraph(style=normal_style)
...
def generate_docx_from_data(data):
    ...
    # (Replaces the old doc.add_picture + centered name approach.)
    table = doc.add_table(rows=1, cols=3)
    table.autofit = False
    # set reasonable widths so center content truly centers
    ...
    # Center: Name (big, colored) + contact line
    ...
    contact_p = center_cell.add_paragraph(contact_info)
...
def generate_pdf_from_data(data):
    if data.get("pamtenLogoBase64"):
        data["pamtenLogoSrc"] = data["pamtenLogoBase64"]
    else:
        data["pamtenLogoSrc"] = None

    if data.get("profilePicBase64"):
        data["profilePicSrc"] = data["profilePicBase64"]
    else:
        data["profilePicSrc"] = None
